    ...
    while (1) {
        ...
        if (sSocket < 0) {
            ...
            for (NSDictionary * cSocket in clients) {
                NSDictionary * dicM = @{@"from":@"Server",@"msg":[NSString stringWithFormat:@"%s已上线",ip]};
                NSString * strWelC = [self jsonString:dicM];
                char  * welCome = [strWelC cStringUsingEncoding:NSUTF8StringEncoding];
                if(send([cSocket[@"socket"] intValue],welCome,strlen(welCome),0)<0)
                {
                    NSLog(@"发送给：%@失败\n",cSocket[@"ip"]);
                }
            }
    ...
    if ([dicS isKindOfClass:[NSDictionary class]]) {
        ...
        while(length = recv([dicS[@"socket"] intValue],buffer,BUFFER_SIZE,0)){
            if (length < 0)
            {
                ...
                [dicM setObject:dicS[@"ip"] forKey:@"from"];
                [dicM setObject:[NSString stringWithCString:buffer encoding:NSUTF8StringEncoding] forKey:@"msg"];
                NSString * strResend = [self jsonString:dicM];
                char  * resendMsg = [strResend cStringUsingEncoding:NSUTF8StringEncoding];
                //转发给其他客户端
                for (NSDictionary * cSocket in clients) {
